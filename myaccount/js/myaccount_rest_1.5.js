...
//    data['searchString'] = options.search || '';
  ...
  this.getWishlistItem = function (wishlistOid, options) {
    ...
    jQuery.ajax({
      url: '/rest/myaccount/wishlist/' + wishlistOid,
      async: (options.success || options.failure) ? true : false,
      headers: { "cache-control": "no-cache" },
      cache: false,
      dataType: 'json'
  ...
  this.updateWishlistItem = function (wishlistItem, options) {
    ...
    jQuery.ajax({
      url: '/rest/myaccount/wishlist/' + wishlistItem.wishlistOid,
      type: 'put',
      data: JSON.stringify(wishListItem),
      async: (options.success || options.failure) ? true : false,
      headers: { "cache-control": "no-cache" },
...
//              updatedWishlistItem = result;
  ...
  /**
   * returns auto orders, if any.
   * @param [options] success and failure callbacks
   * @return Object no callbacks specified, this returns back an array of auto orders, if any, else null on failure.
   */
  this.getAutoOrders = function (options) {
  ...
  /**
   * cancels an auto order
   * @param autoOrderOid the id of the auto order
   * @param [options] success and failure callbacks
   */
  this.cancelAutoOrder = function (autoOrderOid, options) {

    options = options || {};

    jQuery.ajax({
      url: '/rest/myaccount/autoOrders/' + autoOrderOid + "/cancel",
      type: 'post',
      async: (options.success || options.failure) ? true : false,
      headers: { "cache-control": "no-cache" },
      cache: false,
  ...
  /**
   * updates an auto order shipment date
   * @param autoOrderOid the id of the auto order
   * @param desiredShipmentDts the shipment date as a string in YYYY-MM-DD format
   * @param [options] success and failure callbacks
   */
  this.updateAutoOrderShipmentDate = function (autoOrderOid, desiredShipmentDts, options) {

    options = options || {};

    jQuery.ajax({
      url: '/rest/myaccount/autoOrders/' + autoOrderOid + "/updateShipmentDate/" + desiredShipmentDts,
      type: 'post',
      async: (options.success || options.failure) ? true : false,
      headers: { "cache-control": "no-cache" },
      cache: false,

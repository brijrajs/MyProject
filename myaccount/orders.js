...
app.models.Order = Backbone.Model.extend({
  'idAttribute': 'orderId',
  url: function () {
    return '/rest/myaccount/orders/' + this.get('orderId');
  }
});
...
//    console.log('order ' + this.model.get('orderId') + ' render');
    ...
    context['enableCaseManagement'] = window.enableCaseManagement ? true : false;
  ...
  'redirectToReview': function (event) {
    ...
    location.href = "/cgi-bin/UCReviewItem?merchantId=" + encodeURIComponent(this.model.get('merchantId')) + "&itemId=" + encodeURIComponent(itemId);
  },
  ...
  // TODO: get this to toggle open/closed with the button.
  'showComments': function () {
    var that = this;
    if (this.model.get('orderCase')) {
      // load up all the messages.
      myAccount.getOrderCaseMessages(this.model.get('orderId'), {
        success: function (messages) {
          that.model.get('orderCase').messages = messages;
          that.render();

          jQuery('.case-container', that.$el).removeClass('hide');
      ...
      jQuery('.case-container', this.$el).toggleClass('hide');
    }
  ...
  'sendFeedback': function () {
    ...
    if (hasCase) {
      myAccount.insertOrderCaseMessage(this.model.get('orderId'), message, {
      ...
      myAccount.insertOrderCase(this.model.get('orderId'), orderCase, {
  ...
  'showTracking': function () {
    ...
    myAccount.getOrderTracking(orderId, {
...
jQuery(document).ready(function () {
  ...
  // TODO Replace this with he showComments function when it's working
  // showCase();
  // function showCase() {
  //   $(document).on('click', '.js-display-case-container', function(event) {
  //     event.preventDefault();
  //     $(this).siblings('.case-container').toggleClass('hide');
  //   });
  // }
